...
logger = logging.getLogger("connector.github.transformers.pull_request")

class GitHubPullRequestTransformer(BaseDocumentTransformer):
    """Transforms raw Pull Request payloads to Discovery Engine Document format with mapped corporate ACLs and threaded discussions content."""
    ...
    def transform(self, data: RawPayload, context: PipelineContext) -> Optional[discoveryengine.Document]:
        ...
        if not doc_id or not pr_number or not repo_full_name:
            return None
        ...
        # 2. Compile PR Content and thread discussion history
        content_lines = [
        ...
        # Ingest review timeline items
        reviews = payload.get("reviews", [])
        if reviews:
            content_lines.append("--- Reviews ---")
            for r in reviews:
                author_login = r.get("author", {}).get("login", "anonymous")
                content_lines.append(f"- [{r.get('createdAt', '')}] {author_login} ({r.get('state', '')}): {r.get('body', '')}")
                
        compiled_text = "\n".join(content_lines)
        
        web_base = _get_web_base_url(self.file_transformer.client.base_url)
        source_url = f"{web_base}/{repo_full_name}/pull/{pr_number}"
